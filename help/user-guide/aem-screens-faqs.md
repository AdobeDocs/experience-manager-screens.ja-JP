...
### 2.デフォルトのチャネルまたはスケジュールを作成することで、AEM Screensのグレースクリーンの問題を解決する方法

フィールドに空白またはグレーの画面が表示されないようにするには、デフォルトのグローバルチャネルまたはスケジュールを作成し、優先度が最も低い1を持つすべてのディスプレイに割り当ててください。 コンテンツの更新で問題が発生した場合は（ネットワーク、プレーヤー、サーバーまたはレプリケーションが原因で）、プレーヤーがこのコンテンツを既にディスクにキャッシュしているので、正常に再生され、グレーの画面は表示されません。

チャネルやスケジュールなど、他のすべてのコンテンツの優先度は1より大きいので、他のコンテンツは優先され、グローバルチャネルやスケジュールコンテンツ（優先度1）はフォールバックオプションとしてのみ再生されます。
