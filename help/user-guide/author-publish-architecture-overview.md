...
したがって、オーサーインスタンスはすべてのパブリッシュインスタンスにメッセージを送信します。ただし、デバイスに転送されるメッセージは 1 つだけです。適切なメッセージングを確実におこなうために、パブリッシュインスタンス間で何らかの通信が必要になります。This is achieved using *Apache ActiveMQ Artemis*. 各発行インスタンスは、OakベースのSling検出サービスを使用して疎結合されたトポロジに配置され、各発行インスタンスが通信して単一のメッセージキューを作成できるようにActiveMQが設定されます。 Screens デバイスは、ロードバランサーを介してパブリッシュファームをポーリングし、キューの先頭からコマンドを取得します。
...
* ワークフローランチャーの設定では、発行インスタンスで変更されたノードをリッスンし、ワークフローをトリガーして、デバイスの応答を発行インスタンスのアウトボックスに配置します。
...
デバイスはロードバランサーに ping を送信し、パブリッシュインスタンスにルーティングされます。The status of the device is then exposed by the publish instance through a Publish API served @ **api/screens-dcc/devices/static** for all active devices and **api/screens-dcc/devices/&lt;device_id>/status.json** for a single device.
...
デバイスがオーサー環境に登録されると、デバイス設定とチャネル／スケジュールの割り当てが AEM パブリッシュインスタンスにレプリケートされます。次に、AEM Screens デバイス設定が、AEM パブリッシュファームの前面にあるロードバランサーを指すように更新されます。これは1回限りの設定で、画面デバイスが公開環境に正常に接続されると、作成者環境からのコマンドを引き続き受け取ることができ、画面環境を作成者に直接接続する必要はありません。
