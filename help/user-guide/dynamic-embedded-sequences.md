...
>
>動的埋め込みシーケンスは、埋め込みシーケンスと似ていますが、使用する際には階層に従うことができます。つまり、あるチャネルに対して行われた変更や更新が、関連する他のチャネルにも反映されます。 動的埋め込みシーケンスは、親子階層に従い、その中には画像やビデオなどのアセットも含まれます。
>
>***動的埋め込みシーケンス***&#x200B;ではロケーション固有のコンテンツを表示できるのに対して、***埋め込みシーケンス***&#x200B;では、コンテンツの一般的なスライドショーのみ表示できます。 さらに、動的埋め込みシーケンスのセットアップ時に、チャネルの役割と名前を使用してチャネルを設定します。 実践的に実装するには、以下の手順を参照します。
>
...
1. **各ロケーションフォルダーに 2 つのロケーションと 1 つのチャネルを作成します。**
   ...
   1. 「**Region 1**」をクリックし、アクションバーの「**+ 作成**」をクリックします。
   1. ウィザードから「**ロケーション**」をクリックし、「**タイトル**」を「**Store 1**」に設定します。 同様に、ウィザードから別のロケーションを「**Store 2**」として作成し、「**タイトル**」を「**Store 2**」に設定します。 「**Store 1**」と「**Store 2**」の作成時には、「**名前**」フィールドを空のままにしておくことができます。
   1. 手順（b）を繰り返し、今度はウィザードから「**シーケンスチャネル**」をクリックします。 このチャネルの「**タイトル**」に「**Region A**」、「**名前**」に「**region**」と入力します。
...
1. **各ロケーションにディスプレイとチャネルを作成します。**
   ...
   1. 「**Store 1**」をクリックし、アクションバーの「**+ 作成**」をクリックします。
   1. ウィザードから「**ディスプレイ**」をクリックし、**`Store1Display`** を作成します。
   1. 手順（b）を繰り返し、今度はウィザードから「**シーケンスチャネル**」をクリックします。 「**タイトル**」を「**`Store1Channel`**」と入力し、「**名前**」を「**store**」と入力します。
   ...
   前述の手順に従って、チャネルを作成し、**`Store 3`**&#x200B;および&#x200B;**`Store 4`**&#x200B;の&#x200B;**地域B**&#x200B;に表示できます。繰り返しますが、チャネル **`Store3Channel`**&#x200B;と&#x200B;**`Store4Channel`**&#x200B;をそれぞれ作成する際に、**store**&#x200B;と同じ&#x200B;**Name**&#x200B;を使用するようにしてください。
...
1. **スケジュールにチャネルを割り当てます。**
   ...
   1. **割り当てられたチャネル**&#x200B;パネルの「**+ チャネルを割り当て**」をクリックして、**チャネルの割り当て**&#x200B;ダイアログボックスを開きます。
   1. パスごとに「**チャネルを参照**」をクリックします。
   1. 「**チャネルパス**」で **Demo**／***Channels***／***Global*** をクリックします。
   1. 「**チャネルロール**」に「**GlobalAdSegment**」と入力します。
   1. 「**サポートされているイベント**」として、「**最初の読み込み**」、「**待機中画面**」、「**ユーザーインタラクション**」をクリックします。
   1. 「**保存**」をクリックします。

   **地域のチャネルをロール別に割り当てます。**

   1. **割り当てられたチャネル**&#x200B;パネルで「**+ チャネルを割り当て**」をクリックします。
   1. チャネルの割り当てダイアログボックスで、名前で「**参照チャネル**」をクリックします。
...
1. **各ディスプレイへのスケジュールの割り当て**
   ...
   1. 「スケジュール」のパスをクリックします（例：**Demo**／**Schedules**／**AdSchedule**）。
   1. 「**保存**」をクリックします。
...
1. **グループの作成**
   ...
   同様に、**Region-Author**&#x200B;と&#x200B;**Store-Author**&#x200B;など、他の2つのグループを作成します。
...
1. **ユーザーの作成とグループへのユーザーの追加**
   ...
   1. 「**ユーザーを作成**」をクリックし、**ID**&#x200B;に「**グローバルユーザー**」と入力します。
   ...
   同様に、**Region-User**&#x200B;と&#x200B;**Store-User**&#x200B;など、2つの他のユーザーを作成し、それぞれ&#x200B;**Region-Author**&#x200B;と&#x200B;**Store-Author**&#x200B;に追加します。
...
1. **寄稿者へのすべてのグループの追加**
   ...
   1. **Adobe Experience Manager** に移動します。
   1. **ツール**／**セキュリティ**／**グループ**&#x200B;をクリックします。
   1. リストから「**投稿者**」をクリックし、「**メンバー**」タブをクリックします。
   1. 寄稿者に対して、**Global-Author**、**Region-Author**、**Store-Author** などの&#x200B;**グループ**&#x200B;をクリックします。
   1. 「**保存して閉じる**」をクリックします。

1. **各グループの権限へのアクセス**

   1. *User admin* に移動し、この UI を使用して様々なグループの権限を変更します。
...
1. **各グループの権限の変更**
   ...
   **Region-Author の場合：**
   ...
   次の図は、リージョンユーザーが&#x200B;**リージョン A**&#x200B;と&#x200B;**リージョン B**&#x200B;の両方にアクセスできることを示しています。また、4つの店舗（つまり、**Store 1**、**Store 2**、**Store 3**、および&#x200B;**Store 4**&#x200B;すべてにアクセスできますが、**Global** チャネルにはアクセスできません。
