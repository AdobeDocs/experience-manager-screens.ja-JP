...
1. **各ロケーションにディスプレイとチャネルを作成します。**
   ...
   1. を選択 **ストア 1** を選択して、 **+作成** アクションバーから。
   1. を選択 **表示** ウィザードからの作成 **`Store1Display`**.
   1. 上記の 2 番目の手順を繰り返し、今度はウィザードから「**シーケンスチャネル**」を選択します。を入力 **タイトル** as **`Store1Channel`** および **名前** as **store**.
...
1. **チャネルのそれぞれのロケーションにコンテンツを追加します。**
   ...
   に移動します。 **デモ** > **場所** > **地域 A** > **地域 A** を選択して、 **編集** アクションバーから。 チャネルに追加するアセットをドラッグ＆ドロップします。
   ...
   同様に、に移動します **デモ** > **場所** > **地域 B** > **地域 B** を選択して、 **編集** 次に示すように、アクションバーからアセットをチャネルにドラッグ&amp;ドロップします。
...
1. **スケジュールを作成します。**
   ...
   移動して選択 **スケジュール** AEM Screens プロジェクトのフォルダー。 次に、を選択します **作成** アクションバーから。
...
1. **スケジュールにチャネルを割り当てます。**
   ...
   1. に移動します。 **デモ** > **スケジュール** > **AdSchedule** を選択して、 **Dashboard** アクションバーから。
   1. を選択 **+ チャネルを割り当て** から **割り当てられたチャネル** パネルを開く **チャネル割り当て** ダイアログが表示されます。
   1. を選択 **参照チャネル** パス。
   1. 「」を選択します **チャネルパス** as **デモ** > ***チャネル*** > ***グローバル***.
...
1. **各ディスプレイへのスケジュールの割り当て**
   ...
   1. ダッシュボードで、を選択します。 **...** から **割り当てられたチャネルとスケジュール** パネルを開き、 **+スケジュールの割り当て**.
   1. スケジュールのパスを選択します（ここでは、 **デモ** > **スケジュール** > **AdSchedule**）に設定します。
   1. 「**保存**」を選択します。
...
1. **グループの作成**
   ...
   1. を選択 **ツール** > **セキュリティ** > **グループ**.
   1. を選択 **グループを作成** を入力します **Global-Author** 。対象： **ID**.
   1. 「**保存して閉じる**」を選択します。
...
1. **ユーザーの作成とグループへのユーザーの追加**
   ...
   1. **Adobe Experience Manager** に移動します。
   1. を選択 **ツール** > **セキュリティ** > **ユーザー**.
   1. を選択 **ユーザーを作成** を入力します **Global-User** 。対象： **ID**.
   1. このユーザーのパスワードを「**パスワード**」に入力し、パスワードの確認入力もおこないます。
   1. 「」を選択します **グループ** tab キーを押して、にグループ名を入力します。 **グループを選択**&#x200B;例えば、 **Global-Author** 追加 **Global-User** その特定のグループに。
   1. 「**保存して閉じる**」を選択します。
...
1. **各グループの権限へのアクセス**
   ...
   1. を検索 **Global-Author** を選択して、 **権限** タブを付けます（下図を参照）。
