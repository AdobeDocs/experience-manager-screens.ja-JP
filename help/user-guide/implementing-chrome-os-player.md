...
1. Chrome OS デバイスをドメインに登録し、デバイスが Chrome 管理コンソールと同期するまで 15 分間待ちます。Chrome デバイスの登録について詳しくは、 [こちら](https://support.google.com/chrome/a/answer/1360534?hl=ja).
...
1. Scroll down to **キオスク設定** を選択して、 **キオスクアプリケーションの管理**.
...
1. を選択 **デバイス管理** > **Chrome 管理** > **アプリ管理**. AEM Screens Player がリストに表示されます。
1. アプリケーションを選択 **AEM Screens Player**.
1. を選択 **キオスク設定** 組織を選択します（*テスト環境を使用する場合*）に設定します。
1. を選択 **設定ファイルをアップロード** を作成し、設定ポリシー（*JSon ファイル*）に設定します。
1. 「**保存**」を選択します。デバイスを再起動して、ポリシーを同期します。
