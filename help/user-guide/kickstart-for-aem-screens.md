...
1. Adobe Experience Manager（AEM）インスタンスに移動し、次の項目を選択します **スクリーン**. または、`https://localhost:4502/screens.html/content/screens](https://localhost:4502/screens.html/content/screens` から直接アクセスすることもできます。

1. を選択 **Screens プロジェクトの作成** そのため、Screens プロジェクトを作成できます。
1. タイトルに「」と入力 **DemoScreen**&#x200B;を選択してから、 **保存**.
...
1. プロジェクトを作成したら、 **DemoScreen** をプロジェクトおよび選択 **チャネル** フォルダー（下図を参照）。 を選択 **+作成** アクションバーから。

   ![画像](assets/kickstart/demo-2.png)

1. を選択します。 **シーケンスチャネル** ウィザードからを選択します。 **次**.
...
1. を選択 **編集** アクションバーから（下図を参照）。 **TestChannel** のエディターが開きます。

   ![画像](assets/kickstart/demo-6.png)

1. アクションバーの左側のサイドパネルを切り替えるアイコンを選択して、アセットとコンポーネントを開きます。
...
1. を選択 **+作成** アクションバーから。
1. を選択 **場所** ウィザードからを選択します。 **次**.
1. を入力 **名前** あなたの場所の場合（タイトルをに入力します） **TestLocation**）に設定し、 **作成**.
...
1. を入力 **タイトル** as **LobbyDisplay** を選択して、 **作成**.
...
>1. を選択 **AEM Screens** Chrome プレーヤーを起動するためのプラグイン。 デフォルトでは、プレーヤーはフルスクリーンモードで起動します。押す **Esc** をクリックしてフルスクリーンモードを終了します。

Chrome OS プレーヤーがオンになったら、次の手順に従って Chrome デバイスを登録します。

1. AEM インスタンスから、プロジェクトの&#x200B;**デバイス**&#x200B;フォルダーに移動します。

1. 「」を選択します **デバイスマネージャ** アクションバーから。

   ![画像](assets/kickstart/demo-register1.png)

1. 「」を選択します **デバイスの登録** 右上から。

1. 必要なデバイスを選択し、 **デバイスの登録**.
...
1. 次の場合 **登録コード** 両方のマシンで同じものを選択 **Validate** をAEMで行います。

1. 目的の名前をに設定します。 **ChromeDeviceforDemo** デバイスの場合、を選択します。 **登録**.

   ![画像](assets/kickstart/demo-register4.png)

1. を選択 **ディスプレイを割り当て** から **デバイスの登録に成功しました** ダイアログが表示されます。

   ![画像](assets/kickstart/demo-register5.png)

1. ディスプレイへのパスをとして選択します **DemoScreen** > **場所** > **TestLocation** > **LobbyDisplay** を選択して、 **割り当て**.
