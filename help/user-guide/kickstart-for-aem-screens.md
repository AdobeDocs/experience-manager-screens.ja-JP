...
description: このページでは、デモAEM Screensプロジェクトを作成します。 これは、インストールから新しいプロジェクトの設定から、AEM Screens Player でのコンテンツの表示に至るまで、デジタルサイネージエクスペリエンスを作成する際に役立ちます。
...
1. アクションバーの左側にあるサイドパネルを切り替えるアイコンをクリックして、アセットとコンポーネントを開きます。
...
>[!NOTE]
>***場所*** は、様々なデジタルサイネージエクスペリエンスを区分するもので、各種スクリーンの場所に応じたディスプレイ設定が含まれています。
...
>[!NOTE]
>***表示*** は、1 つまたは複数のスクリーンで実行されるデジタルエクスペリエンスを表します。
...
1. 次の場所から必要なディスプレイに移動します。 **DemoScreens** > **場所** > **TestLocation** > **LobbyDisplay**.
...
1. ディスプレイへのパスを次のように選択します。 **DemoScreens** > **場所** > **TestLocation** > **LobbyDisplay** をクリックします。 **割り当て**.
