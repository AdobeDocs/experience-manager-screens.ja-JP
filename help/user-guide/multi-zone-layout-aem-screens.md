...
1. クリック **1x2 分割画面チャネル** から **作成** ウィザード。
...
1. **チャネルにコンテンツを追加する**
   ...
   1. 「」をクリックします **MultiZone** チャネルとクリック **編集** アクションバーから。
...
1. **チャネルフォルダーの作成**
   ...
   1. クリック **チャネルフォルダー** から **作成** ウィザードを表示して、 **次**.
...
1. **チャネルフォルダーにさらに 2 つのチャネルを追加する**
   ...
   1. クリック **シーケンスチャネル** から **作成** という名前のチャネルの作成ウィザード **`Zone1`**.
   1. クリック **`Zone1`** をクリックして、 **編集** アクションバーから。
...
1. **メインチャネル（MultiZone）に埋め込みシーケンス（コンポーネント）を追加する**
   ...
   1. ゾーンの 1 つに埋め込まれたシーケンスをクリックします。
   1. エディターで、埋め込みシーケンスの&#x200B;**設定**（レンチ）アイコンをクリックします。
   1. チャネルパスをとしてクリックします **ゾーン** > **チャネル** > **EmbeddedChannels** > **`Zone1`**&#x200B;を参照してください（下図を参照）。
...
1. **ロケーションの作成**
   ...
   1. 「」をクリックします **場所** フォルダーをクリックし、 **作成** アクションバーから。
   1. クリック **場所** から **作成** ウィザードを表示して、 **次**.
   1. 「**タイトル**」に「**SanJose**」と入力し、「**作成**」をクリックします。

1. **ディスプレイの作成**

   1. に移動します。 **ゾーン** > **場所** フォルダー。
   1. 「」をクリックします **サンノゼ** 場所とクリック **作成** アクションバーから。
...
1. **ディスプレイへのチャネルの割り当て**
   ...
   1. 「」をクリックします **ロビー** 表示してクリック **チャネルを割り当て** アクションバーから。
...
1. **デバイスの登録**
   ...
   1. クリック **デバイスの登録** リストから保留中のデバイスをクリックします。

      >[!NOTE]
      > デバイスのタイトルは、「**デバイスの登録**」タブに表示されるデバイストークン（「**トークン**」フィールド）と一致する必要があります。

   1. タイトルがデバイストークンと一致する場合は、デバイスをクリックし、 **デバイスの登録** アクションバーから。
   ...
   1. を入力 **タイトル** as **`Chrome-Device1`** をクリックして、 **登録**.
   1. クリック **ディスプレイを割り当て** デバイス設定へのパスをクリックします。
