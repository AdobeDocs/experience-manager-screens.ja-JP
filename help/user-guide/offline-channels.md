...
>[!NOTE]
> カスタムオフラインリソースハンドラーのテンプレートの詳細と、その特定のプロジェクトの最小要件については、『AEM Screens用カスタムコンポーネントの開発』の「 `pom.xml`[Template for Custom Handlers](/help/user-guide/developing-custom-component-tutorial-develop.md#custom-handlers)****」を参照してください。