...
AEM Screens からは、コンテンツの公開と非公開をおこなうことができます。「公開を管理」機能を使用すると、コンテンツの更新をオーサーからパブリッシュ経由でデバイスに配信できます。AEM Screens プロジェクト全体のコンテンツを公開／非公開することもできますし、チャネル、ロケーション、デバイス、アプリケーション、スケジュールのいずれか 1 つのコンテンツを公開／非公開することもできます。
...
1. プロジェクトをパブリッシュスタンスに公開するために、アクションバーの「**公開を管理**」をクリックします。
...
1. **公開を管理**&#x200B;ウィザードで「**公開**」をクリックします。
   ...
   > [!CAUTION]
   > 作成者またはコンテンツの作成者が、作成者インスタンスに関連付けられているデバイスでの変更を確認する場合は、チャネルダッシュボードから「オフラインコンテンツを更新 **** 」をクリックするか、プロジェクトを選択します。 この場合、オフラインコンテンツの更新は、作成者インスタンスでのみ実行されます。
...
1. プロジェクトに移動し、アクションバーの「**オフラインコンテンツを更新**」をクリックします。このアクションは、同じコマンドをパブリッシュインスタンスに転送するので、パブリッシュインスタンスにもオフライン zip ファイルが作成されます。
   ...
   >[!NOTE]
   >
   >パブリケーションの管理ワークフローを完了し、作成者インスタンスを指すプレーヤーが存在する場合は、作成者のオフラインコンテンツの更新をトリガする必要があります。これにより、作成者インスタンスでオフラインで更新が作成されます。

   >[!CAUTION]
   >
   >プレーヤーがオーサーバーに登録されている場合は、オーサーインスタンスでオフラインコンテンツの更新をトリガーする必要があります。 発行インスタンスに登録されているプレーヤーに対しては、オフラインコンテンツの更新は必要ありません。
...
1. チャネルをパブリッシュスタンスに公開するために、アクションバーの「**公開を管理**」をクリックします。
...
1. チャネルダ **ッシュボードで「オフラインコンテンツの更新をトリガー** 」を選択すると、オフラインコンテンツが作成者インスタンスに対してのみプッシュされ、発行インスタンスはプッシュされません。 手順1 ～ 4は、オフラインコンテンツを発行インスタンスにプッシュするためのものです。
