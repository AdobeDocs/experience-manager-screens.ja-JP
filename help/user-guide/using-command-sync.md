...
>[!NOTE]
>同期再生を行うには、すべてのハードウェア・デバイスが同じハードウェア仕様で、好ましくは同じオペレーティング・システムを持つ必要があります。 異なるハードウェアとオペレーティングシステム間の同期はお勧めしません。
...
>[!NOTE]
>マスター・デバイスの同期を解除すると、そのマスター・デバイスに関連付けられているすべてのクライアント・デバイスのリンクも解除されます。
...
1. Click **Confirm** to de-sync the selected device from the master.
   ...
   >[注意:]
   > マスター・デバイスを選択して非同期オプションを使用すると、マスターに接続されているすべてのデバイスは、1回の手順で非同期になります。
