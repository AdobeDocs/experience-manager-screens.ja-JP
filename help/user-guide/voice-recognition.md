...
>**プライバシーに関する重要な情報**
>
>音声認識機能を使用する場合は、お住まいの地域に適用されるすべての法的および倫理的ガイドラインに従ってください。 これらのガイドラインには、エンドユーザーに対して、プレーヤーが音声認識を使用していることを視覚的に通知することが含まれますが、これに限定されるものではありません）。 Adobeは、音声関連の情報を受け取ったり、保存したり、処理したりするものではありません。 AEM Screens Player は、ブラウジングエンジンに組み込まれた標準の web 音声 API を使用します。 この API はバックグラウンドで、音声をテキストに変換するため、Googleのサーバーに波形の音声を送信します。 プレーヤーは、テキストを設定済みのキーワードと照合します。
>
...
コンテンツ作成者は、音声対応のディスプレイを設定できます。 この機能の目的は、お客様がディスプレイを操作する方法として音声を使用できるようにすることです。 同様の使用例としては、店舗でお勧め商品を探す、食堂やレストランでメニューのアイテムを注文するなどがあります。この機能により、ユーザーのアクセシビリティが向上し、顧客体験を大幅に向上させることができます。
...
音声認識機能を使用する前に、プロジェクトと、プロジェクトにコンテンツを設定したチャネルがあることを確認してください。
...
1. 各チャネルに移動して、コンテンツを追加します。 例えば、次に移動します。 **VoiceDemo** > **チャネル** > **メイン** チャネルをクリックします。 クリック **編集** アクションバーから、必要に応じてコンテンツ（画像/ビデオ）を追加します。 同様に、両方にコンテンツを追加します **コールドドリンク** および **ホットドリンク** チャネル。
   ...
   **HotDrinks**：
   ...
   Screens 分割チャネルをプロジェクトに追加した場合は、次に移動します。 **SplitScreen** をドラッグ&amp;ドロップして、2 つの埋め込みシーケンスを配置します。 両方にパスを追加 **コールドドリンク** および **ホットドリンク** チャネル（下図を参照）。
...
1. に移動します。 **基本** タブをクリックしてから、既存のタグを **タグ** フィールドまたは作成します。
   ...
   プロジェクトに合わせて事前にAEM インスタンスからタグを作成して選択することもできます。 で説明されている手順に従った後 [タグの作成](#creating-tags)の場所からタグをクリックして、チャネルに追加できます（下図を参照）。

   ![画像](assets/voice-recognition/vr-tag1.png)

1. 同様に、というタイトルのタグを追加します。 **ホット** に **ホットドリンク** チャネル。
...
1. 「」をクリックします **VoiceDemo** プロジェクトしてクリック **タグを作成** アクションバーから。
...
1. ディスプレイにチャネルを割り当てたら、に移動します **LobbyDisplay** ディスプレイをクリックします。 アクションバーの「**プロパティ**」をクリックします。

1. に移動します。 **表示** tab キーを押して **音声有効化** オプション： **コンテンツ**.
...
この **メイン** チャネルがコンテンツを再生しています。 ただし、キーワードと共に単語を使用する場合は **ホット**（例：） *温かい飲み物をいただきたいのですが*&#x200B;チャンネルでのコンテンツの再生が開始されます。 **ホットドリンク** チャネル。

同様に、キーワードで単語を使用する場合 **寒冷** 例： *何か冷たいものがほしいのですが*&#x200B;チャンネルでのコンテンツの再生が開始されます。 **コールドドリンク** チャネル。

**分割画面チャネルに対する目的の出力**

この **メイン** チャネルがコンテンツを再生しています。 ただし、キーワードと共に単語を使用する場合は **ホット** および **寒冷** 以下のように連携する *温かい飲み物と冷たい飲み物のメニューを見たいのですが*&#x200B;その場合、チャネルは次のコンテンツを再生 **SplitScreen** チャネル。 次のような場合： *メインメニューに戻る*&#x200B;に戻ります。 **メイン** チャネル。
