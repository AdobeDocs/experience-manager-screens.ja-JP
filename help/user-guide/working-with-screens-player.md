...
* AEM Screens Player に対して保持するログファイルの最大数を、「**保持するログファイルの 最大数**」で指定します。
...
>[!NOTE]
>
>**非推奨：**&#x200B;このハードウェア選択ガイダンスは非推奨（廃止予定）であり、新しいデプロイメントには使用しないでください。 ハードウェアの仕様と推奨事項は、特定のデプロイメント要件とユースケースに基づいて、適格なAV インテグレーターによって提供する必要があります。
