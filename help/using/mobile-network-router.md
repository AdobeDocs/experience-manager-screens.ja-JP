...
この設定の利点は、モバイルルーターを最適化された場所に配置して、利用可能な最高のネットワークカバレッジを確保できることです。この部分は通常、高く開いた位置にあり、周囲のコンクリートや金属構造はできるだけ少なくなります。
AEMに接続するために固定回線は必要ないため、この設定はAEM Screens Screen ユーザーに柔軟性を提供します。また、一時的またはモバイルの設定にも興味があります。
...
*十分な*は、接続されているAEM Screens デバイスの数によって異なります。また、スマートフォン、タブレット、レジ、コンピューター、ゲスト Wi-Fi ネットワークなど、ネットワーク内の他の消費者の使用状況によっても異なります。
すべてのデバイスがインターネット接続に同時にアクセスし、帯域幅が線形的に減少し、ネットワークに追加される消費者やコンピューターが増えていることに注意してください。
特定の理論的なネットワーク接続に加えて、モバイルルーターのカバレッジが少なくとも*good*&#x200B;であることを確認する必要があります。また、基盤となる月次プランは、接続されたLAN内のすべての接続クライアントにサービスを提供するのに十分なデータ容量と十分な帯域幅をカバーする必要があります。
...
AEM Screensは、デジタルサイネージユーザーに大きな利点をもたらします。画像やビデオなど、必要なすべてのメディアファイルをダウンロードしてローカルに保存します。この考え方により、特定の画面に表示される新しいコンテンツがある場合、主要なネットワークトラフィックが発生しています。
通常の操作では、頻繁に更新されない定義されたプレイリストを持つことで、すべてのファイルがプレーヤーに保存されたときに、ネットワークに依存しない操作を提供します。
センサーやその他のトリガーとのインタラクションが多く、コンテンツが動的な場合は、迅速で信頼性の高いネットワーク接続が即座のスクリーン反応に不可欠です。可能な限り優れた顧客体験を実現します。
次の表では、予想されるパフォーマンスと潜在的な待ち時間に対するネットワーク接続キーデータの意味を詳しく説明します。
